...
export default function ExpenseList({ expenses, theme, onDelete }: any) {
  // 1. Guard against empty data or loading states
    ...
    return (
      <div className={`${theme.card} p-12 rounded-[40px] border ${theme.border} text-center mt-8 animate-in fade-in`}>
        <p className={`${theme.subtext} font-black uppercase text-[10px] tracking-widest`}>
          No transactions recorded yet for this expedition.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 mt-8 animate-in fade-in slide-in-from-bottom-4">
      <div className="flex items-center justify-between px-4">
        ...
        <p className={`${theme.subtext} text-[8px] font-bold uppercase`}>
      ...
      <div className="space-y-3">
        {expenses.map((exp: any) => (
          <div 
            key={exp.id} 
            className={`${theme.card} p-6 rounded-[32px] border ${theme.border} flex flex-col md:flex-row md:items-center justify-between gap-4 hover:shadow-md transition-all group`}
          >
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-1">
                  <span className={`${theme.subtext} text-[9px] font-black uppercase flex items-center gap-1`}>
                    <User size={10} /> {exp.paid_by_name}
                  </span>
                  <span className="w-1 h-1 rounded-full bg-slate-200" />
                  <span className={`${theme.subtext} text-[9px] font-black uppercase flex items-center gap-1`}>
                    <Users size={10} /> 
                    {exp.split_with && exp.split_with.length > 0 ? `${exp.split_with.length} People` : 'Everyone'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between md:justify-end gap-6 border-t md:border-none pt-4 md:pt-0">
              <div className="text-right">
                ...
                <div className="flex items-center justify-end gap-1 text-slate-400">
                  ...
                  <p className="text-[8px] font-bold uppercase">
                    {/* Prioritize custom capture date over system timestamp */}
              ...
              <button 
                ...
                className="p-3 rounded-xl hover:bg-red-50 hover:text-red-500 text-slate-200 transition-all"
              >
                <Trash2 size={18} />
              </button>
            </div>
